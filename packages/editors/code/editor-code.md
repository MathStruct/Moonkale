...
- **Word wrap** (spec 014): `editor.wrap` from `Settings`; `backend::mount(.., wrap, ..)` and `set_wrap` → the bundle's `Compartment` around `EditorView.lineWrapping`; `editor.toggleWrap` (Alt+Z) flips the user setting, and every panel's settings effect pushes it into its view.
