...
ALL="menubar session graph links-sqlite terminal typst lsp ladybug agent search-trace settings rich agent-writes flow wasm-ext phone palette files replace lsp2 git auth history presence graph3d duckdb wiki highlight panels image shell server extensions claude-code agent-server terminal-native"
reset() {
