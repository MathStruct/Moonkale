...
## Milestone 11 — remote
- `main()` installs `api::relay` and sets the server URL to it before `launch` (P-098). `MOONKALE_REMOTE=http://host:port` (+ `MOONKALE_TOKEN`) at start → `api::client::connect`; every platform callback (`open_folder`, `attach_source`, `spawn_terminal`, `compile_typst`, `spawn_lsp`, `git_local`, `wasm_ext::{list_any, run_any}`) dispatches on `api::client::active()`. The LLM provider stays local.
- `open_remote(host, path, sink)` → `moonkale_remote::SshSession::open` with `server_binary()`; `RemoteHandle` implements `ui::remote::RemoteSession` (close kills the master `ssh`). `ssh_hosts()` reads `Host` aliases (no wildcards, no `!`) from `~/.ssh/config` for the dialog (`ssh_hosts_in` is unit-tested). `ssh_at_start()`: `MOONKALE_SSH='[VAR=v] [options] host:/path'` or `--ssh …` opens the remote folder when the window starts (the last word is `host:path`, the rest is passed to `ssh` — `--ssh "SSH_AUTH_SOCK=0 -p 443 daniel@192.168.178.62:/home/daniel/Code"`) (before the recent-folder reopen). `WorkspaceConfig::remote = Some(RemoteHosts { open, hosts, at_start })`.
- `tests/remote.rs` (ignored): the client wiring against a running server.
- Running the release app from a script on the KDE Wayland session (P-099): `env -i HOME=$HOME PATH=$PATH XDG_RUNTIME_DIR=/run/user/$UID DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/$UID/bus WAYLAND_DISPLAY=wayland-0 DISPLAY=:1 XAUTHORITY=<session xauth> ./moonkale`; screenshots with `spectacle -b -n -f -o out.png`.

## Milestone 12
`server_client()` → `ui::ServerClient` over `api::client` (*File → Connect to Server…*); `moonkale-llm` with `claude-code`; `llm_provider` needs no key for `claude-code`; `WorkspaceConfig::agent_sessions` with `api::client::agent_available` (server sessions while connected, when `agent.on_server` is set).
