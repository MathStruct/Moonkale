...
- **Second halves (Milestone 9):** History → **Compact** / **Restore**; presence cursors need nothing extra; the desktop joins a hub with `MOONKALE_HUB=http://host:port` (and `MOONKALE_TOKEN` if the server has one). DuckDB is built in on desktop and the server (`moonkale-sources-sql/duckdb`, bundled; ~3 min extra on a clean build): click a `.csv`/`.tsv`/`.parquet` to get its folder as tables, or a `.duckdb` file. **Android**: `export ANDROID_HOME=~/Android/Sdk ANDROID_NDK_HOME=$ANDROID_HOME/ndk/<ver> JAVA_HOME=/usr/lib/jvm/java-17-openjdk`, `rustup target add aarch64-linux-android x86_64-linux-android`, then `cd packages/mobile && dx serve --platform android` with a device (`adb devices`) or an AVD.
