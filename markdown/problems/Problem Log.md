...
| P-061 WebKitWebProcess SIGSEGV in NVIDIA EGL on desktop start | Every desktop run since the graph renderer exists leaves a core dump: `WebKitWebProcess` crashes inside `libnvidia-eglcore` (called from WebKitGTK's compositor, no Moonkale frames on the stack) — the known WebKitGTK DMA-BUF renderer × proprietary NVIDIA problem. The app keeps working because WebKit respawns the process. `desktop/main.rs` now sets `WEBKIT_DISABLE_DMABUF_RENDERER=1` before launch when `/proc/driver/nvidia/version` exists (`MOONKALE_KEEP_DMABUF=1` opts out). Needs Daniel's confirmation that the dumps stop and the graph still reports `gl` ([[Linux Desktop Setup]]) | resolved (pending confirmation) | 3 |
