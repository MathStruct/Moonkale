...
Tiers ([[Extension System]]): **core** — always on, cannot be disabled; **optional** — on by default, can be switched off in *Settings → Extensions*; **opt-in** — off until enabled. *Static* = a Rust crate compiled into every build and registered in `ui::default_extensions()`; *wasm* = a module loaded at runtime (JSON ABI v1: commands and agent tools only).
...
| `dev.moonkale.editor-agent` | Agent | `editors/agent` (1 026 + 330) + `llm` (2 239 + 500) | optional | chat panel, tools under the policy gate, transcripts as pages, MCP for external agents (server); **server sessions** (`agent.on_server`, Milestone 12) that finish without a client; the **`claude-code` provider** (the `claude` CLI on the subscription) | ✅ | ⚙️ | ⚙️ (server sessions when connected) | an LLM provider: Claude Code / Anthropic / OpenAI-compatible / Ollama / mock, keys via `SecretRef` |
