...
> [!info] Status — Milestone 9 done, Moonkale runs on a phone
> **Second halves.** History you can act on (compaction into snapshots, **Restore** any earlier text as an unsaved edit with provenance); presence with **cursor lines** in the editor gutter and a **desktop hub client**; node dragging in 3D; **DuckDB** — `.duckdb` files and folders of CSV/TSV/Parquet as queryable tables. And the first **Android** build: the release APK runs on a Galaxy S10e — phone shell, editor with the soft keyboard, save + history, graph on WebGL2 (five Android-only fixes, P-087–P-091). See [[Milestone 9 - Implementation Log]]. Before that, Milestone 8 —
