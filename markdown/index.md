...
> [!info] Status — Milestone 9 done (Android pending)
> **Second halves.** History you can act on (compaction into snapshots, **Restore** any earlier text as an unsaved edit with provenance); presence with **cursor lines** in the editor gutter and a **desktop hub client**; node dragging in 3D; **DuckDB** — `.duckdb` files and folders of CSV/TSV/Parquet as queryable tables. The Android build waits for the NDK and a device. See [[Milestone 9 - Implementation Log]]. Before that, Milestone 8 —
