...
`packages/mobile/build-android.sh` → `dist/moonkale-0.1.0-android-arm64.apk`, **45 MB** (23 MB in Milestone 9: the phone now carries `api` + the client relay with rustls, the Rust terminal's `vt100` and the Rust code editor's 20 tree-sitter grammars). Built here in 3 minutes. **Installed on the Galaxy S10e the same night** (`build-android.sh install`): the app starts with the phone's own vault and the earlier notes; *File → Connect to Server…* against a server on the LAN (`--bind 0.0.0.0 --port 8443 --token-stdin`, `MOONKALE_INSECURE_HTTP=1`) first did nothing — P-116, a scope-owned task cancelled by the closing dialog — and after the fix showed `⇅ http://192.168.178.62:8443`, the server's 176-file folder as a second source and opened its `Getting Started.md`. An `android` job (`setup-android` with NDK 29, JDK 17, the same script) joined `release.yml`; it is untested there like the other non-Linux jobs.
