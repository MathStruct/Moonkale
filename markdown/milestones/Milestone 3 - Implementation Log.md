...
- **P-061 WebKitWebProcess crashes in NVIDIA EGL** (core dumps reported by Daniel): reproduced — the web process dies inside the driver while tearing down a live WebGL context, i.e. on every exit and every `dx serve` rebuild kill. Not a startup crash and not the DMA-BUF renderer. Since the process is exiting anyway, the launcher now sets `RLIMIT_CORE=0` on NVIDIA machines (opt-out `MOONKALE_COREDUMPS=1`); verified no dump after a kill, and Daniel confirmed the KDE crash popup is gone under `dx serve`.
