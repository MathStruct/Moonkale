...
- **P-072 The last folder was not reopened at launch** (Daniel's first desktop test): restore ran only after a manual open. Desktop now reopens `recent_folders[0]` at start (`reopen_last_folder`); tracing added on the restore path. Confirmed by Daniel afterwards (folder, tabs and layout come back as left). My own desktop runs had coincided with a locked KDE session (WebKit suspends invisible pages), so they were inconclusive — lesson: check the lock screen before trusting a desktop experiment.
